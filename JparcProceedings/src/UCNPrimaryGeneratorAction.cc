...
void UCNPrimaryGeneratorAction::GeneratePrimaries(G4Event* anEvent)
{
  ...
  thePrimaryParticleName = particleGun->GetParticleDefinition()->
                                                GetParticleName() ;
  G4double x0 = 59.9*mm, y0 = 0.0*mm, z0 = 0.0*mm;//
  G4double px=0, py=3.0e+00, pz=0 ;//m/sec
	if(Clockwise)py*=-1;//

  G4double n_energy = 0;
  //G4double massnew = 1.674927e-27;     //neutronmasse in kg
  G4double massnew = 1.674927351e-27;     //neutronmasse in kg
  G4double p_length = 0;
  ...
  G4ThreeVector spin(0.0,1.0,0.0);//(0.,0.,G4UniformRand()-.5);
  //spin=spin.unit();
  ...
  p_length = sqrt(px * px + py * py + pz * pz);//Actually, it corresponds to the velocity
  //n_energy = massnew * p_length * p_length / 2 * 6.242e18 * 1e9;
  n_energy = massnew * p_length * p_length / 2 * 6.24150934e18 * 1e9;//[neV]

  G4cout << "UCNPrimaryGenerator: velocity length = " << p_length << ", Ekin0 = " << n_energy << "neV"<<G4endl;
  G4cout << "velocity" << px << ", " << py << ", " << pz << G4endl  ;
  G4cout << "pos " << x0 << ", " << y0 << ", " << z0 << G4endl;

  
  G4ThreeVector momentum(px,py,pz);//To calculate the momentum direction with the velocity
