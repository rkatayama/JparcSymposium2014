...
{ 
  ...
  RotationCmd = new G4UIcmdWithAnInteger("/gun/rotation",this);//
  RotationCmd->SetGuidance(" e.g. /gun/rotaion 0, which means ucn rotates anti-clockwise");//
  RotationCmd->SetGuidance(" otherwise, ucn rotates clockwise");//
  RotationCmd->SetParameterName("choice",true);//
  RotationCmd->SetDefaultValue(0) ; //
...
{ 
  ...
  if( command == RotationCmd)//
   { UCNAction->SetRotation(RotationCmd->GetNewIntValue(newValue));}//
}
