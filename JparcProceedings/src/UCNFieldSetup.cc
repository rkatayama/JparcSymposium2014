...
//#include "UCNTimedependendField.hh"
...
UCNFieldSetup::UCNFieldSetup()
  : fChordFinder(0), fStepper(0), fIntgrDriver(0),EFieldDirection(0),GravityEffect(1)
{
 fEMfield = new UCNUniformGravField(G4ThreeVector(0.0, 0.0, 4.8979e-9*6.673e-2*volt/cm));

 fFieldMessenger = new UCNFieldSetupMessenger(this) ;  

 fEMfield->GetUCNFieldSetupPointer(this);//add by katayama
...
{    
  ...
  fEMfield = new UCNUniformGravField(pFV);

  fFieldMessenger = new UCNFieldSetupMessenger(this) ;  

	fEMfield->GetUCNFieldSetupPointer(this);//
...
UCNFieldSetup::~UCNFieldSetup()
{
	...
	G4cout<<"EFieldDirection="<<((EFieldDirection==1)?"Eup":"Edown")<<G4endl;
	G4cout<<"GravityEffect="<<((GravityEffect==1)?"On":"Off")<<G4endl;
...
void UCNFieldSetup::SetFieldValue(G4ThreeVector fieldVector)
{
  ...
  { 
    ...
    fEMfield = new  UCNUniformGravField(fieldVector);
