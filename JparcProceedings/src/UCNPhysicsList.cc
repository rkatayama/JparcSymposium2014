...
#include "UCNTimeStopStep.hh"//add by katayama
...
UCNPhysicsList::~UCNPhysicsList()
{
  delete physicsListMessenger; 
	//add by katayama
	/////////////////////
  delete theMaterialBoundary;
  delete theSimpleAbsorption;
  delete theSimpleLoss;
  delete theMultiScattering;
  delete theSpin;
  delete theDetector;
  delete theDecayProcess;
	delete theTimeStopStep;
	/////////////////////
}
...
void UCNPhysicsList::ConstructUCN()
{
  // Add UCN processes
	//change by katayama
	//////////////////////////
  theMaterialBoundary = new UCNMaterialBoundary();
  theSimpleAbsorption = new UCNSimpleAbsorption();
  theSimpleLoss = new UCNSimpleLoss();
  theMultiScattering = new UCNMultiScattering();
  theSpin = new UCNSpin();
  theDetector = new UCNDetector();
  theDecayProcess = new G4Decay();
	//////////////////////////

	//UCNTimeStopStep* theTimeStopStep = new UCNTimeStopStep();//add by katayama
	theTimeStopStep = new UCNTimeStopStep();//add by katayama
  ...
  while( (*theParticleIterator)() )
  {
    ...
    if (particleName == "ucn")       {  
      ...
      pmanager->AddDiscreteProcess(theTimeStopStep); //add by katayama
      ...
      pmanager ->SetProcessOrdering(theTimeStopStep, idxPostStep); //add by katayama
